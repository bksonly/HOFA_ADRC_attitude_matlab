function [sys,x0,str,ts,simStateCompliance] = attitude_controller_2usin(t,x,u,flag,dt,Rd,wd,dwd,J,K)
...
switch flag,
  ...
  %%%%%%%%%%%
  case 3,
    sys=mdlOutputs(t,x,u,dt,Rd,wd,dwd,J,K);
...
%=============================================================================
%
function sys=mdlOutputs(t,x,u,dt,Rd,wd,dwd,J,K)
i=fix(t/dt)+1;
iRd=Rd(:,:,i);
iwd=wd(i,:)';
idwd=dwd(i,:)';
...
% qd=quaternion(iRd, 'rotmat', 'point');
% qe=conj(q)*qd;
% qe=compact(qe);
% iwd=10*sign(qe(1))*qe(2:4)';
% idwd=[0;0;0];
...
tmp1=Rr*hat_ew*hat(w)-hat(iwd)*Rr*hat_ew-hat(idwd)*Rr;
tmp=tmp1-tmp1';
...
if rank(RB)==3
    B=RB*inv(J);
    invB=J*inv(RB);
    M=invB*(-vee(tmp)-K*[e;de])+cross(w,J*w);
%     M=invB*(-vee(tmp))+cross(w,J*w);
else
